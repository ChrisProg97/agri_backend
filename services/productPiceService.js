...
const addCropProductPrice = async (crop_id, market_id, price, added_by) => {
  ...
  const updateCropProductPrice = async (id, crop_id, market_id, price) => {
    try {
      const result = await db.one(
        'UPDATE crop_product_prices SET crop_id = $1, market_id = $2, price = $3 WHERE id = $4 RETURNING id, crop_id, market_id, price',
...
const addAnimalProductPrice = async (animal_id, market_id, price, added_by) => {
    ...
    try {
      const query = `
        SELECT 
          ...
          markets.name AS market_name, 
          prices.price
        FROM animal_product_prices
        JOIN crops ON prices.animal_id = crops.id
        JOIN markets ON prices.market_id = markets.id
      `;
  ...
  // Getting the details according to the market and Animal  Produc tPrices
  ...
  // Update a price by ID
  const updateAnimalProductPrice = async (id, animal_id, market_id, price) => {
    try {
      const result = await db.one(
        'UPDATE animal_product_prices SET animal_id = $1, market_id = $2, price = $3 WHERE id = $4 RETURNING id, animal_id, market_id, price',
        [animal_id, market_id, price, id]
      );
      return result;
    } catch (error) {
